...
Now we introduce the plugins system to be able to add, remove, print, normalize every file/directory into the fitnessm2eapp during the installation.
...
| plugin.js    | `module.exports = { async apply(value, previousValues) { // Add code here }}` | The `apply` function is the one that apply the refactoring of the fitnessm2eapp. `value` is the result of the prompt, `previousValues` is previous responses of previous prompts and it's defined like `{ <plugin_name>: <value> }`. |
...
module.exports = {
  async apply(value, previousValues) {__
    return new Promise(resolve => {
      if (value) {
        console.log('TheCodingMachine React-Native FitnessM2EApp initialized the TYPESCRIPT please wait !',)
      } else {
        console.log('TheCodingMachine React-Native FitnessM2EApp initialized the JAVASCRIPT please wait !',)
      }
      resolve()
    })
  },
